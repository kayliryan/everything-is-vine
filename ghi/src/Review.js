...
export default function Review() {
  ...
  let tax = 0.0725 * (floatSubTotalSum - discount);
      ...
      <List disablePadding>
        {cartItems.map((cartItem) => (
          <ListItem key={cartItem.brand} sx={{ py: 1, px: 0 }}>
            <ListItemText
              primary={`${cartItem.year} ${cartItem.brand}`}
              secondary={cartItem.varietal}
            />
            <Typography variant="body2">{`${cartItem.cust_quantity} @ $${
              cartItem.price
            } = $${cartItem.cust_quantity * cartItem.price}`}</Typography>
          </ListItem>
        ))}
